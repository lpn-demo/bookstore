...
export default class {
	...
	@action add = () => {
		...
		this.api.add(sendData).then((response) => {
			this.rootStore.products.loadData();
			this.clear();
			this.rootStore.notification.add('newProductAdd', 'success');
		});
	}
	...
	@action update=(id)=>{
		...
		this.api.update(id,sendData).then((response) => {
			this.rootStore.products.loadData();
			this.rootStore.notification.add('productUpd', 'success');
		});
	}

	@action remove=(id)=>{
		this.api.remove(id).then((res) => {
			this.rootStore.products.remove(id);
		});
	}
